...
created: 2022-06-22
modified: 2025-07-09
published: 2026-05-16T01:30:26.726+08:00
---

# 🃏 卡片笔记 (Cards) 说明

从 daily note、`Sources/Clippings`、Sources 或其他整理过程沉淀来的内容，可以是临时卡片，也可以是[[永久笔记]]，不管怎么称呼。这里的笔记，我可能通过以下几种方式进行纳管：
...
这个系统，和[[P.A.R.A]]不冲突。如果第一次创建文件就能有 PARA 的归属，就直接放到对应 PARA 文件夹里面。如果没有归属，比较离散，就先放这，或一直放这。这里相当于[[自下而上]]笔记结构的池子，等时机成熟、数量够多以后，优先外迁到 `Spaces/` 或由 `Atlas/MOCs/` 承接，不再在 `Cards/` 下面继续按主题硬分二级目录。
...
一些临时的、pending 的又不舍得删除的笔记先留在 `Cards/` 根层；等主题成熟后再移动到对应 Area / Resource，或者只用 MOC 组织，不为了整齐把 `Cards/` 人工拆成多个主题文件夹。
