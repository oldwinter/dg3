...
test("read-later labels use the central locale catalog", () => {
  ...
  assert.equal(enUs.components.readLater.exportList, "Copy all as Markdown checklist")
  assert.equal(enUs.components.readLater.filter, "Filter saved titles")
  assert.equal(zhCn.components.readLater.noMatches, "没有匹配的标题")
  assert.equal(zhTw.components.readLater.filter, "按標題篩選筆記")
